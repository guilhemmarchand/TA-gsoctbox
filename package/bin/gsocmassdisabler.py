...
# set logging
filehandler = RotatingFileHandler('%s/var/log/splunk/gsoctbox_gsocmassdisabler.log' % splunkhome, mode='a', maxBytes=10000000, backupCount=1)
...
class GsocMassDisabler(GeneratingCommand):
    ...
    mode = Option(
        ...
        **Description:** value for mode.''',
        require=False, default="simulation", validate=validators.Match("mode", r"^(simulation|live)$"))
    ...
    es_forbidden_apps = Option(
        ...
        **Description:** es_forbidden_apps, a comma separated list of apps which enabled scheduled are forbidden on ES SHC.''',
        require=True, default=None, validate=validators.Match("es_forbidden_apps", r"^.*$"))
    ...
    adhoc_forbidden_apps = Option(
        ...
        **Description:** es_forbidden_apps, a comma separated list of apps which enabled scheduled are forbidden on Adhoc SHC.''',
        require=True, default=None, validate=validators.Match("adhoc_forbidden_apps", r"^.*$"))
    ...
    def generate(self, **kwargs):
        ...
        if self:
            ...
            logging.info("search=\"{}\", earliest=\"{}\", latest=\"{}\"".format(kwargs_oneshot.get('earliest_time'), kwargs_oneshot.get('latest_time'), search))
            ...
            try:
                ...
                logging.error("failed to run the provider search with exception=\"{}\"".format(str(e)))
                raise Exception("failed to run the provider search with exception=\"{}\"".format(str(e)))
            ...
            for record in savedsearches_list:
                ...
                logging.debug("investigating app=\"{}\", savedsearch_name=\"{}\", owner=\"{}\", disabled=\"{}\"".format(record_app, record_savedsearch_name, record_owner, record_disabled))
                ...
                if es_sh_layer:
                    ...
                    if record_app in es_forbidden_apps and record_disabled == 0:
                        ...
                        logging.info("the savedsearch_name=\"{}\" is currently enabled, disabled=\"{}\", app=\"{}\", this search is part of a forbidden application in this Search Layer and will be disabled".format(record_savedsearch_name, record_disabled, record_app))
            ...
            if len(process_action) > 0:
                ...
                for record in process_action:
                    ...
                    if not self.mode == 'simulation':
                        ...
                        try:
                            disabled_results = report_update_enablement(session_key, record.get('app'), record.get('savedsearch_name'), 'disable')
            ...
            logging.info("gsocmassdisabler has terminated, run_time=\"{}\"".format(str(time.time() - start)))
