...
filehandler = RotatingFileHandler(
    f"{splunkhome}/var/log/splunk/gsoctbox_gsocgennotable.log",
    mode="a",
    maxBytes=10000000,
    backupCount=1,
)
...
class GsocGenNotable(GeneratingCommand):
    ...
    def generate(self, **kwargs):
        ...
        # Define the query
        search = f"""| makeresults 
        | eval search_name="{self.search_name}", rule_name="{self.rule_name}", rule_title="{self.rule_title}", orig_host_bunit="{self.orig_host_bunit}", severity="{self.severity}", rule_description="{self.rule_description}", orig_alert_category="{self.orig_alert_category}" 
        ...
        try:
            ...
            oneshotsearch_results = self.service.jobs.oneshot(search, **kwargs_oneshot)
            ...
            logging.error(
                f'failed to call the custom command with exception="{str(e)}"'
            )
            raise ValueError(
                f'failed to call the custom command with exception="{str(e)}"'
            )
